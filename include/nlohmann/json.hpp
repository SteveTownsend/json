...
#undef JSON_IMPLEMENT_OPERATOR
  ...
  public:
    ...
    JSON_HEDLEY_WARN_UNUSED_RESULT
    static basic_json diff(const basic_json& source, const basic_json& target, const string_t& path = "")
    {
        // the patch
        ...
        switch (source.type())
        {
            case value_t::array:
            {
                ...
                while (i < source.size())
                {
                    ...
                    // indices
                    result.insert(result.begin() + end_index, object({{"op", "remove"}, {"path", detail::concat<string_t>(path, '/', detail::to_string<string_t>(i))}}));
                    ++i;
                }
